...
export const syncLibraries = async (configId: string): Promise<LibraryRecord[]> => {
  try {
    ...
    for (const library of libraries) {
      ...
      // Get item count from Jellyfin library object
      // Jellyfin's VirtualFolders API provides ItemCount directly
      let itemCount = (library as any).ItemCount || 0;
      
      // If ItemCount is not available or is 0, try to get it by querying items
      // Use the library's ItemId if available (this is the actual folder ID)
      if (itemCount === 0 && (library as any).ItemId) {
        try {
          const libraryItemId = (library as any).ItemId;
          console.log('[JellyfinLibraries] ItemCount is 0, trying to query items for library:', libraryItemId);
          
          // Query items using the library's ItemId as parentId
          ...
          console.log('[JellyfinLibraries] Queried item count:', itemCount);
        } catch (queryError: any) {
          console.warn('[JellyfinLibraries] Could not query item count for library', libraryId, ':', queryError.message);
          // Keep itemCount as 0 or from library object
        }
      }
      
      console.log('[JellyfinLibraries] Library item count:', { 
        libraryId, 
        libraryName, 
        collectionType,
        itemCount,
        libraryItemId: (library as any).ItemId,
        libraryItemCount: (library as any).ItemCount
      });
      ...
      console.log('[JellyfinLibraries] Processing library:', {
        ...
        ItemCount: itemCount,
        CollectionTypeDetermined: collectionType,
      });

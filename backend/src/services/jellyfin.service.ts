...
const cache = new NodeCache({ stdTTL: 30, checkperiod: 60 }); // 30 seconds default TTL, check every 60 seconds
...
export class JellyfinService {
  ...
  async getItems(params: {
    ...
    // Enable caching for all queries with short TTL for movies/series (30 seconds)
    // This balances performance (fast loading) with freshness (removed items disappear quickly)
    const isMovieOrSeries = params.includeItemTypes === 'Series' || params.includeItemTypes === 'Movie';
    const cacheTTL = isMovieOrSeries ? 30 : 300; // 30 seconds for movies/series, 5 minutes for others
    ...
    const cacheParams = { ...queryParams };
      ...
      if (cached) {
        console.log('[JellyfinService] Returning CACHED data:', { total: cached.TotalRecordCount, itemsCount: cached.Items?.length });
        return cached;
      }
    
    console.log('[JellyfinService] Fetching fresh data from Jellyfin API...');
    ...
    try {
      ...
      // DISABLED: Item validation is too slow and causes 7-10 second delays
      // Jellyfin's API is generally reliable, and removed items will be filtered out on next cache refresh
      // If validation is needed, it should be done asynchronously in the background
  ...
  async getItemDetails(itemId: string, validateCache: boolean = true): Promise<JellyfinItem> {
    ...
    // Enable caching for item details with short TTL (30 seconds) for performance
    const cacheKey = `item_${itemId}`;
    const cached = cache.get<JellyfinItem>(cacheKey);
    if (cached) {
      console.log('[JellyfinService] Returning cached item details:', itemId);
      return cached;
    }
    ...
    try {
      ...
      // Cache item details for 30 seconds for performance
      cache.set(cacheKey, item, 30);
      return item;
    } catch (error: any) {
  ...
  getImageUrl(itemId: string, imageType: string = 'Primary'): string {
    // Return relative path only (no protocol/host for proxy/CDN compatibility)
    // Images are proxied through backend at /api/media/images/:itemId/:imageType
    return `/api/media/images/${itemId}/${imageType}`;
  }

  getStreamUrl(itemId: string): string {
    // Return relative path only
    return `/api/media/stream/${itemId}/master.m3u8`;
  }

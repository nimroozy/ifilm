...
export default function Series() {
  ...
  useEffect(() => {
    // Apply filtering and sorting to current page series
    let result = [...series];
  ...
  const loadSeries = async () => {
    ...
    try {
      ...
      const response = await api.get(`/media/series?page=${currentPage}&limit=${itemsPerPage}`);
      ...
      if (seriesData.length > 0) {
        ...
        // Extract unique genres from current page (accumulate across pages)
        ...
        {!loading && !error && (
          <div className="flex flex-wrap items-center gap-4 mb-8">
            ...
            <div className="text-[#B3B3B3] ml-auto">
              Showing {((currentPage - 1) * itemsPerPage) + 1}-{Math.min(currentPage * itemsPerPage, totalSeries)} of {totalSeries} shows
            </div>
          </div>
        )}
